"""Prepare a menu image for vision processing.

画像の Exif を削除し、必要に応じて縮小して、Skill 側の Vision 解析へ
渡せる状態にする。API 呼び出しは行わない。
"""
...
def default_output_path(path: Path) -> Path:
    ...
    元ファイル名に `.preprocessed` を付けた同一ディレクトリのパスを返す。
    """
...
def preprocess_image(path: Path, out_path: Path | None = None) -> dict[str, Any]:
    """Strip Exif data and resize a menu image.

    Pillow で画像を読み込み、GPS 等の Exif を保存せずに書き出す。
    """
    try:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    入力画像と任意の出力先を受け取る。
    """
    parser = argparse.ArgumentParser(description="Prepare a menu image for Claude/GPT Vision processing.")
