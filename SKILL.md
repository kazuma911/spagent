...
4. **ウェルカムメニュー (起動時デフォルト)** — 初回発話が明確な指示 (「メニュー作って」「取り込んで」「タイム投入」等) **でない** 場合 (例: `spagent` / `こんにちは` / `何ができる？` / `今日は何しますか` / 空発話) は、以下 8 択メニューを提示する:
   - **初回起動時 (sessions/ が空 かつ groups.json 未登録)**: 8 択の上に「まずは ⑤ 初期セットアップから始めるのがおすすめです」の一言を添える
   - **2 回目以降の起動時**: `sessions/` の最新ディレクトリの日付とタイトル (menu.tsv の `# Title:` 行) を「前回: <日付> <タイトル>」形式で 1 行添える。存在しなければ省略
   - **8 択メニュー**:
     ```
     👋 こんにちは、コーチ！今日は何をしますか？
     (前回: <YYYY-MM-DD> <セッションタイトル>)  ← 2 回目以降

     【よく使う】
     ① 今日/明日のメニューを作る                → Workflow A
        例: 「明日 8/25 のメニュー作って」
     ② 練習後の記録・フィードバック              → Workflow B
        例: 「今日のタイム表を投入」「メモ画像から起こして」
     ③ 過去メニューを検索する                   → Workflow C
        例: 「Threshold 系の 3000m 帯を見せて」

     【シーズン設計】
     ④ 長期プランを立てる                       → Workflow D
        例: 「冬季 JO までの 12 週計画を作って」
     ...
     ```
   - **明確な指示があった場合はウェルカムメニューを表示せず、直接該当 Workflow へルーティング** (Step 0 の要求タイプ判定を優先)
   - **判定基準**: 発話に Step 0 のキーワード (「メニュー」「タイム」「投入」「Threshold」「12 週計画」「セットアップ」「プロファイル追加」「取り込み」等) が含まれない、かつ **20 文字以内の短い挨拶 / 質問**、または番号のみ (①〜⑧) → ウェルカム表示
...
- **テンプレ整備** (Workflow F.templates) — W-up / Drill / Kick / C-down テンプレを事前整備する独立モード。Workflow A の Step 11 の C/D/E (テンプレ編集 / 新規作成 / ゼロから作成) 相当を、メニュー作成の流れとは切り離して起動できる:
  1. **対象種別選択** — W-up / Drill / Kick / C-down のいずれか (複数選択可)
  2. **一覧提示** — `templates/<種別>/*.md` を一覧表示 (id / name / applicable_conditions / 総距離 / tags)
  3. **アクション選択**:
     - **編集**: 既存テンプレを対話で修正 → `templates/<種別>/<id>.md` を上書き
     - **叩き台に新規作成**: 既存テンプレをコピー → 新 id/name で対話編集 → 新規保存
     - **ゼロから作成**: 対話でセット内容 / 総距離 / 目安時間 / applicable_conditions / 目的コメント を順に入力 → 新規保存
     - **削除**: 廃止テンプレを対話確認の上で削除
  4. **フロントマター自動更新** — 保存時に `updated` / `updated_by` を追記
  5. **将来拡張**: Workflow G から自動抽出モード (過去メニューの W-up/Drill 部を抽出 → クラスタリング → テンプレ自動生成) を追加予定
